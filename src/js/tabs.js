export class Tabs {
  ...
  _setAccordionState(parent, elements, controls) {
    ...
    elements.forEach((element, idx) => {
      const accordion = this._createDOMElement('div', {
        'data-tabs': 'accordion',
      });
      const accordionWrapper = this._createDOMElement('div', {
        'data-tabs': 'accordion-wrapper',
      });
      const accordionContent = this._createDOMElement('div', {
  ...
  _removeAccordionState(parent, elements, controls) {
    ...
    const controlList = this._returnScopeChild(
      parent.querySelectorAll('[data-tabs="controls"]'),
      parent,
    );
    const content = this._returnScopeChild(
      parent.querySelectorAll('[data-tabs="content"]'),
      parent,
    );
    const activeAccordions = this._returnScopeList(
  ...
  _initTab(tab) {
    ...
    if (accordionMedia && !tab.accordionListener) {
      this.accordionBreakpointChecker(
        accordionMedia,
        tab,
        tabElements,
        tabControlElements,
      ...
      accordionMedia.addEventListener(
        'change',
        this.accordionBreakpointChecker.bind(
          this,
          accordionMedia,
          tab,
          tabElements,
